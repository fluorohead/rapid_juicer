...
|TGA|Targa Graphics Adapter Image|Уже сильно устаревший формат, чаще встречающийся на платформе DOS. **Из-за нечёткой сигнатуры снижает производительность, поэтому рекомендуется включать только по необходимости**. Rapid Juicer'ом распознаются true-color разновидности (тип 2 и тип 10).|
