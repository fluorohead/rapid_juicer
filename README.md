...
Приложение поддерживает многозадачность, т.е. можно запустить несколько сканирований одновременно, но не более 16. По понятным причинам не рекомендуется делать этого с одним и тем же файловым хранилищем, т.к. в этом случае прироста производительности не будет. 

Работа начинается с формирования задания путём добавления путей к отдельным файлам или сразу к каталогам. В одно задание можно добавить не более 1024 путей. Если в вашем каталоге более 1024 файлов, то просто добавьте целый каталог, а не файлы по-отдельности. Одно задание (сессия сканирования) работает в рамках отдельного потока операционной системы. Если вы запустили задачу, то приложение готово сформировать следующую и запустит его в своём отдельном потоке, не зависимом от первого.
