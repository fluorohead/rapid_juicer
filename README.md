...
Блок кнопок "STOP/PAUSE/SKIP" управляет процессом поиска и позволяет соответственно : останавливать без возможности возобновления, ставить на паузу с возможностью мгновенного возобновления, и пропускать текущий файл.
...
Кнопка "save selected" под таблицей результатов отдельно взятого формата позволяет, соответственно, сохранить только явно выбранные результаты - клик по таблице выделяет строку зелёным.

![r_j_screenshot](./docs/selected.jpg)

REPORT - сохраняет html-отчёт по найденным ресурсам с подробным описанием каждого ресурса. Закрытие окна сессии не происходит, чтобы можно было выполнить последующий SAVE ALL. Отчёт удобно сохранять для изучения статистики, не запуская более долгий процесс сохранения найденного.
